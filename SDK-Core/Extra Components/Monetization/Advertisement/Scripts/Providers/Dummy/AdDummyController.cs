...
namespace Watermelon
{
    public class AdDummyController : MonoBehaviour
    {
        ...
        [Space]
        [SerializeField] GameObject bannerCenterObject;
        ...
        private RectTransform bannerCenterRectTransform;

        private void Awake()
        {
            bannerRectTransform = (RectTransform)bannerObject.transform;
            bannerCenterRectTransform = (RectTransform)bannerCenterObject.transform;

            // Toggle editor visibility
            bannerObject.ToggleVisibility(true);
            bannerObject.TogglePicking(true);            
            
            bannerCenterObject.ToggleVisibility(true);
            bannerCenterObject.TogglePicking(true);
        ...
        public void Initialise(AdsSettings settings)
        {
            switch (settings.DummyContainer.BannerPosition)
            {
                ...
                case BannerPosition.Center:
                    bannerCenterRectTransform.pivot = new Vector2(0.5f, 0.5f);

                    bannerCenterRectTransform.anchorMin = new Vector2(0.0f, 0.5f);
                    bannerCenterRectTransform.anchorMax = new Vector2(1.0f, 0.5f);

                    bannerCenterRectTransform.anchoredPosition = Vector2.zero;
                    break;
            }
        }
        ...
        public void ShowBannerCenter()
        {
            bannerCenterObject.SetActive(true);
        }

        public void HideBannerCenter()
        {
            bannerCenterObject.SetActive(false);
        }
