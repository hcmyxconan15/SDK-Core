...
namespace Watermelon
{
    ...
    public class AdMobHandler : AdProviderHandler
    {
        ...
        private BannerView bannerCenterView;
        ...
        public override void HideBannerCenter()
        {            
            if (bannerCenterView != null)
                bannerCenterView.Hide();
        }

        public override void DestroyBannerCenter()
        {            
            if (bannerCenterView != null)
                bannerCenterView.Destroy();
        }
        ...
        private void RequestBannerCenter()
        {
            // Clean up banner before reusing
            if (bannerCenterView != null)
            {
            ...
            switch (adsSettings.AdMobContainer.BannerCenterType)
            {
                case AdMobContainer.BannerPlacementType.Banner:
                    adSize = AdSize.Banner;
                    break;
            ...
            AdPosition adPosition = AdPosition.Bottom;
            switch (adsSettings.AdMobContainer.BannerCenterPosition)
            {
                case BannerPosition.Bottom:
                    adPosition = AdPosition.Bottom;
                    break;
            ...
            bannerCenterView = new BannerView(GetBannerCenterID(), adSize, adPosition);

            // Register for ad events.
            bannerCenterView.OnBannerAdLoaded += HandleAdLoaded;
            bannerCenterView.OnBannerAdLoadFailed += HandleAdFailedToLoad;
            bannerCenterView.OnAdPaid += HandleAdPaid;
            bannerCenterView.OnAdClicked += HandleAdClicked;
            bannerCenterView.OnAdFullScreenContentClosed += HandleAdClosed;

            // Load a banner ad.
            bannerCenterView.LoadAd(GetAdRequest());
        }
        ...
        public override void ShowBannerCenter()
        {
            if (bannerCenterView == null)
                RequestBannerCenter();

            if(bannerCenterView != null)
                bannerCenterView.Show();
        }
    ...
    #region RewardedVideo Callback
        ...
        public string GetBannerCenterID()
        {
#if UNITY_EDITOR
            return "unused";
#elif UNITY_ANDROID
